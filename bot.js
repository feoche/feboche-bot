...
*/
  ...
  const EMOJIS = ['👐', '🙌', '👏', '🙏', '🤝', '👍', '👎', '👊', '✊', '🤛', '🤜', '🤞', '✌', '🤘', '👌', '👈', '👉', '👆', '👇', '☝', '✋', '🤚', '🖐', '🖖', '👋', '🤙', '✍', '💅', '🤳', '🤗'];
  const LINKS = ['http://www.academie-francaise.fr/digital', 'http://www.cnrtl.fr/definition/digital'];
      ...
      if (data.text && data.lang === 'fr') {
        ...
        if (containsRegExp(text, PROHIBITEDWORDS[0].queries)) {
          ...
          if (data.user.screen_name.toLowerCase() !== botUsername.toLowerCase() &&
            ...
            let probability = MINPROBABILITY + ((followers - MINFOLLOWERS) / (MAXFOLLOWERS - MINFOLLOWERS) * (MAXPROBABILITY - MINPROBABILITY));
                    ...
                    (error, statusData) => {
                      ...
                      if (error) {
                          ...
                          twitterAPI.showUser(botUsername, (error, data) => {
                            if (!error) {
                              if (data[0].name.match(/(\[TL\]) (.*)/)) {
                                ...
                                twitterAPI.updateProfile({name: '[TL] ' + data[0].name}, error => {
                                  if (error) {
                        ...
                        if (statusData.user.name.match(/(\[TL\]) (.*)/) !== null) {
                          //DO EET
                          twitterAPI.updateProfile({name: tweetLimitCheck[2]}, error => {
                            if (error) {
                              console.log("error while trying to change username (going OUT of TL)");
                            } else {
                              hasNotifiedTL = true;
